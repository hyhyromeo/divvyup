...
export default function JoinGroup({ onJoined, onBack }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        const { group, participant, error } = await api.joinGroup(code.toUpperCase(), nickname, avatar);
  ...
  return (
    <div className="px-6">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <div>
          <label className="block text-sm font-medium text-slate-600 mb-2">Share Code</label>
          <input 
            type="text" 
            value={code}
        ...
        <div>
          <label className="block text-sm font-medium text-slate-600 mb-2">Your Nickname</label>
          <input 
            type="text" 
            value={nickname} 
